...
//    private RecordedAudioToFileController saveRecordedAudioToFile;
    ...
    private boolean iceHostGet = false;
    private boolean iceStunGet = false;
    private boolean iceTurnGet = false;
    private boolean iceComplete = false;

    private VideoSink frameVideoSink;


    private boolean remoteAudioEnabled = true;
    private int remoteAudioVolume = 100;
    private long callStartedTimeMs;
//    protected VideoSink localProxyVideoSink;
//    protected VideoSink remoteProxyVideoSink;
    private Camera1Capturer camera1Capturer;
//    private String streamUrl;
    ...
    public static class PeerConnectionParameters {
        ...
        public PeerConnectionParameters(boolean audioSendEnabled, boolean videoSendEnabled, boolean audioRecvEnabled, boolean videoRecvEnabled,
                                        boolean loopback, boolean tracing,
            ...
            this.audioSendEnabled = audioSendEnabled;
            this.videoSendEnabled = videoSendEnabled;
            this.audioRecvEnabled = audioRecvEnabled;
            this.videoRecvEnabled = videoRecvEnabled;
    ...
    public interface PeerConnectionEvents {
         ...
         */
        void onLocalDescription(final PeerConnectionClient pc, final SessionDescription sdp);

        /**
         * Callback fired once local Ice candidate is generated.
         */
        void onIceCandidate(final PeerConnectionClient pc, final IceCandidate candidate);

        /**
         * Callback fired once local ICE candidates are removed.
         */
        void onIceCandidatesRemoved(final PeerConnectionClient pc, final IceCandidate[] candidates);

        /**
         * Callback fired once local Ice candidate generate state.
         */
        void onIceGatheringComplete(final PeerConnectionClient pc, SessionDescription sdp);

        /**
         * Callback fired once connection is established (IceConnectionState is
         * CONNECTED).
         */
        void onIceConnected(final PeerConnectionClient pc);

        /**
         * Callback fired once connection is disconnected (IceConnectionState is
         * DISCONNECTED).
         */
        void onIceDisconnected(final PeerConnectionClient pc);

        /**
         * Callback fired once DTLS connection is established (PeerConnectionState
         * is CONNECTED).
         */
        void onConnected(final PeerConnectionClient pc);

        /**
         * Callback fired once DTLS connection is disconnected (PeerConnectionState
         * is DISCONNECTED).
         */
        void onDisconnected(final PeerConnectionClient pc);

        /**
         * Callback fired once peer connection is closed.
         */
        void onPeerConnectionClosed(final PeerConnectionClient pc);

        /**
         * Callback fired once peer connection statistics is ready.
         */
        void onPeerConnectionStatsReady(final PeerConnectionClient pc, final StatsReport[] reports);

        /**
         * Callback fired once peer connection error happened.
         */
        void onPeerConnectionError(final PeerConnectionClient pc, final String description);

        void onDataChannelMessage(final PeerConnectionClient pc, String message);
    }
    ...
    public void setParameters(WXRTCDef.WXRTCVideoEncParam param){
        Size size = RTCUtils.getVideoResolution(param.videoResolution);

        this.peerConnectionParameters.videoWidth = size.getWidth();
        this.peerConnectionParameters.videoHeight = size.getHeight();
        this.peerConnectionParameters.videoFps = param.videoFps;
        this.peerConnectionParameters.videoMaxBitrate = param.videoMaxBitrate;
    }
    ...
    public void startCall(final VideoSink localRender, final VideoSink remoteSink){
    ...
    protected void onConnectedToRoomInternal() {
        final long delta = System.currentTimeMillis() - callStartedTimeMs;

        createOffer();
    }

    private @Nullable VideoCapturer createVideoCapturer(boolean frontCamera) {
        final VideoCapturer videoCapturer;
        Logging.d(TAG, "Creating capturer using camera1 API.");
        videoCapturer = createCameraCapturer(new Camera1Enumerator(false), frontCamera);
...
//            reportError("Failed to open camera");
    ...
    private @Nullable VideoCapturer createCameraCapturer(CameraEnumerator enumerator, boolean frontCamera) {
        ...
        if (targetDeviceName.isEmpty() && deviceNames.length > 0) {
            targetDeviceName = deviceNames[0];
        }
        if (!targetDeviceName.isEmpty()) {
            VideoCapturer videoCapturer = enumerator.createCapturer(targetDeviceName, null);
            if (videoCapturer != null) {
                if (videoCapturer instanceof Camera1Capturer) {
                    camera1Capturer = (Camera1Capturer) videoCapturer;
                }
                return videoCapturer;
            }
        }
    ...
    public boolean isVideoSendEnabled() {
        return peerConnectionParameters.videoSendEnabled;
    }

    public boolean isAudioSendEnabled() {
        return peerConnectionParameters.audioSendEnabled;
    }

    public boolean isVideoRecvEnabled() {
        return peerConnectionParameters.videoRecvEnabled;
    }

    public boolean isAudioRecvEnabled() {
        return peerConnectionParameters.audioRecvEnabled;
    }
...
//                saveRecordedAudioToFile = new RecordedAudioToFileController(executor);
        ...
        if (peerConnectionParameters.videoCodecHwAcceleration) {
            encoderFactory = new DefaultVideoEncoderFactory(
                    rootEglBase.getEglBaseContext(), false /* enableIntelVp8Encoder */, enableH264HighProfile);
...
//                .createIceServer());
        SignalingParameters params = new SignalingParameters(iceServers, offerSdp, iceCandidates);
        signalingParameters = params;
        createPeerConnection(params);
    }

    JavaAudioDeviceModule createJavaAudioDevice() {
...
//                .setSamplesReadyCallback(saveRecordedAudioToFile)
    ...
    private void createMediaConstraintsInternal() {
        ...
        sdpMediaConstraints.mandatory.add(
                new MediaConstraints.KeyValuePair("OfferToReceiveAudio", Boolean.toString(peerConnectionParameters.audioRecvEnabled)));
        sdpMediaConstraints.mandatory.add(new MediaConstraints.KeyValuePair(
                "OfferToReceiveVideo", Boolean.toString(peerConnectionParameters.videoRecvEnabled)));
    }
    ...
    private void createPeerConnectionInternal() {
        ...
        RTCConfiguration rtcConfig = new RTCConfiguration(signalingParameters.iceServers);
...
//        rtcConfig.disableIpv6 = true;
        ...
        if (dataChannelEnabled) {
            ...
            dataChannel.registerObserver(new DataChannel.Observer() {
                ...
                public void onMessage(final DataChannel.Buffer buffer) {
                    ...
                    events.onDataChannelMessage(PeerConnectionClient.this, strData);
                }
            });
        }
        isInitiator = false;
        ...
        if (peerConnectionParameters.videoRecvEnabled) {
            remoteVideoTrack = (VideoTrack) peerConnection.addTransceiver(MediaStreamTrack.MediaType.MEDIA_TYPE_VIDEO).getReceiver().track();
...
//        rootEglBase.release();
    ...
    public boolean isHDVideo() {
        return isVideoSendEnabled() && videoWidth * videoHeight >= 1280 * 720;
    }
    ...
    private void getStats() {
        ...
        boolean success = peerConnection.getStats(new StatsObserver() {
            ...
            public void onComplete(final StatsReport[] reports) {
                events.onPeerConnectionStatsReady(PeerConnectionClient.this, reports);
            }
        }, null);
        if (!success) {
    ...
    public void setLocalVideoTrackEnabled(final boolean enable) {
        executor.execute(() -> {
    ...
    public void setRemoteVideoTrackEnabled(final boolean enable) {
        executor.execute(() -> {
    ...
    public void setRemoteAudioTrackEnabled(final boolean enable) {
        ...
        executor.execute(() -> {
            AudioTrack track = getRemoteAudioTrack();
            if (track != null) {
    ...
    public void setRemoteAudioTrackVolume(final int volume) {
    ...
    public boolean isCameraZoomSupported(){
        if (camera1Capturer != null && camera1Capturer.getOpenedCamera() != null){
            Camera.Parameters parameters = camera1Capturer.getOpenedCamera().getParameters();
    ...
    public void setCameraZoom(int value){
        if (camera1Capturer != null && camera1Capturer.getOpenedCamera() != null){
            ...
            camera1Capturer.getOpenedCamera().setParameters(parameters);
        }
    }
    ...
    public void addLocalIceCandidate(final IceCandidate candidate) {
        executor.execute(() -> {
            if (peerConnection != null && !isError) {
...
//            drainCandidates();
    ...
    public void startVideoSource(boolean frontCamera) {
        executor.execute(() -> {
            if (videoSource == null) {
                return;
            }
            ...
            this.videoCapturer = createVideoCapturer(frontCamera);
    ...
    public void changeVideoSource(int width, int height, int framerate) {
        executor.execute(() -> {
            videoWidth = width;
            videoHeight = height;
            videoFps = framerate;
...
//                Log.e(TAG, "videoFrame ==================="+videoFrame.getRotatedWidth()+","+videoFrame.getRotatedHeight());
                if (frameVideoSink != null) {
...
//        fieldTrials += VIDEO_VP8_INTEL_HW_ENCODER_FIELDTRIAL;
    ...
    public void sendMessage(String message){
        ...
        DataChannel.Buffer returnBuffer = null;
        try {
            returnBuffer = new DataChannel.Buffer(ByteBuffer.wrap(message.getBytes("UTF-8")), false);
        } catch (UnsupportedEncodingException e) {
            e.printStackTrace();
        }
        if (returnBuffer != null) {
            dataChannel.send(returnBuffer);
        }
    }
    ...
    private class PCObserver implements Observer {
        ...
        public void onIceCandidate(final IceCandidate candidate) {
            ...
            executor.execute(() -> {
                Log.d(TAG, "onIceCandidate iceGatheringState: " + peerConnection.iceGatheringState());

                if (peerConnectionParameters.loopback) {
                    addRemoteIceCandidate(candidate);
                }
                events.onIceCandidate(PeerConnectionClient.this, candidate);
                ...
                if (iceHostGet && (iceStunGet || iceTurnGet) && !iceComplete) {
                    iceComplete = true;
                    new Handler(Looper.getMainLooper()).postDelayed(()->events.onIceGatheringComplete(PeerConnectionClient.this, peerConnection.getLocalDescription()), 3000);
                }
            });
        }
        ...
        public void onIceCandidatesRemoved(final IceCandidate[] candidates) {
            ...
            executor.execute(() -> {
                ...
                events.onIceCandidatesRemoved(PeerConnectionClient.this, candidates);
            });
        }
        ...
        public void onIceConnectionChange(final IceConnectionState newState) {
            executor.execute(() -> {
                ...
                if (newState == IceConnectionState.CONNECTED) {
                    events.onIceConnected(PeerConnectionClient.this);
                } else if (newState == IceConnectionState.DISCONNECTED) {
                    events.onIceDisconnected(PeerConnectionClient.this);
...
//                    setRemoteAudioTrackVolume(remoteAudioVolume);
                    ...
                    events.onConnected(PeerConnectionClient.this);
                } else if (newState == PeerConnectionState.DISCONNECTED) {
                    events.onDisconnected(PeerConnectionClient.this);
        ...
        public void onIceGatheringChange(IceGatheringState newState) {
            executor.execute(() -> {
                ...
                if (newState == IceGatheringState.COMPLETE && !iceComplete) {
                    iceComplete = true;
                    events.onIceGatheringComplete(PeerConnectionClient.this, peerConnection.getLocalDescription());
                }
            });
        }
        ...
        public void onCreateSuccess(final SessionDescription origSdp) {
            ...
            if (!sdpDescription.contains("a=extmap-allow-mixed")) {
                ...
                if (inertIndex >= 0) {
                    ...
                    newLinesList.addAll(Arrays.asList(lines).subList(inertIndex, lines.length - 1));
...
//                    events.onIceGatheringComplete(peerConnection.getLocalDescription());
                }
            });
        }
            ...
            executor.execute(() -> {
                ...
                if (isInitiator) {
                    ...
                    if (peerConnection.getRemoteDescription() == null) {
                        ...
                        events.onLocalDescription(PeerConnectionClient.this, localSdp);
                    } else {
