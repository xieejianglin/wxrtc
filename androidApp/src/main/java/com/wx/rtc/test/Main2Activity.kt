...
class Main2Activity : AppCompatActivity(), WXRTCListener {
    companion object {
    ...
    private val mAppId = "123"
    private val mRoomId = "75726410"
    private var localVideoInLocalView = false
    private var isFrontCamera = true
    private var remoteUserId: String? = null
    private var isScreenCapture = false

    private val localVideoView: RTCVideoContainerView?  by lazy { findViewById(R.id.localVideo) }
    private val remoteVideoView: RTCVideoContainerView?  by lazy { findViewById(R.id.remoteVideo) }
...
//    private val zoomOutCamreaButton: Button?  by lazy { findViewById(R.id.btn_zoomout_camera) }
    ...
    private fun init() {
        ...
        params.rotation = WXRTCDef.WXRTC_VIDEO_ROTATION_90
        ...
        changeVideoButton?.setOnClickListener {
            if (mWXRTC.isEnterRoom) {
                ...
                if (localVideoInLocalView) {
                    mWXRTC.updateLocalView(localVideoView)
                    remoteVideoView?.visibility = GONE
                    remoteUserId?.let { userId ->
                        mWXRTC.updateRemoteView(userId, remoteVideoView)
                    }
                } else {
                    mWXRTC.updateLocalView(remoteVideoView)
                    localVideoView?.visibility = GONE
                    remoteUserId?.let { userId ->
                        mWXRTC.updateRemoteView(userId, localVideoView)
                    }
                }
            }
        }
...
//        }
        screenCaptureButton?.setOnClickListener {
            ...
            if (isScreenCapture) {
                ...
                mWXRTC.stopLocalPreview()

                val encParam = WXRTCVideoEncParam()
                encParam.videoResolution = WXRTCDef.WXRTC_VIDEO_RESOLUTION_1920_1080
                encParam.videoResolutionMode = if (resources.configuration.orientation == Configuration.ORIENTATION_PORTRAIT)
                ...
                if (localVideoInLocalView) {
                    mWXRTC.startLocalPreview(isFrontCamera, localVideoView)
                } else {
                    mWXRTC.startLocalPreview(isFrontCamera, remoteVideoView)
                }
            }
        }
...
//                }
    ...
    private fun snapshotVideo(userId: String) {
        mWXRTC.snapshotVideo(userId, object : WXRTCSnapshotListener {
            override fun onSnapshot(userId: String, file: String) {
                Toast.makeText(this@Main2Activity, "${userId}截图成功，文件路径:${file}", Toast.LENGTH_LONG).show()
                snapshotView?.visibility = VISIBLE
                snapshotImage?.setImageBitmap(BitmapFactory.decodeFile(file))
            }
        })
    }
...
//        }
    ...
    override fun onRemoteUserEnterRoom(userId: String) {
        ...
        remoteUserId = userId
        mWXRTC.startRemoteView(userId, remoteVideoView)
